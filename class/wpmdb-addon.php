...
class WPMDB_Addon extends WPMDB_Base {
	...
	function get_wpmdb_basename() {
		global $wpmdb;
		return plugin_basename( $wpmdb->get_plugin_file_path() );
	}

	function meets_version_requirements( $version_required ) {
		$wpmdb_version = $this->get_installed_version( $this->get_wpmdb_basename() );
		$result = version_compare( $wpmdb_version, $version_required, '>=' );
	...
	function version_requirement_warning() { ?>
		<div class="updated warning version-requirement-warning notification-message warning-notice">
			<p>
				...
				<?php
					$wpmdb_basename = $this->get_wpmdb_basename();
